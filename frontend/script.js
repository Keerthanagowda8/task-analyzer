...
function addTaskForm() {
    ...
    const html = `
        <div class="task-input-box">
            ...
            <input type="text" class="input-title" placeholder="e.g. Finish Assignment" oninput="clearError(this)">
            
            <label>Due Date:</label>
            <input type="date" class="input-date" oninput="clearError(this)">
            
            <label>Importance (1-10):</label>
            <input type="number" class="input-importance" min="1" max="10" value="5" oninput="clearError(this)">
            
            <label>Effort (Hours):</label>
            <input type="number" class="input-hours" min="1" value="1" oninput="clearError(this)">
...
async function analyzeTasks() {
    ...
    taskBoxes.forEach(box => {
        ...
        if (!title) {
            titleInput.style.border = "2px solid #ff4444";
            valid = false;
        }
        if (!date) {
    ...
    if (hasError) {
        resultsArea.innerHTML = `
            <div id="error-message" style="background:#ffe6e6; color:#cc0000;
                padding:15px; border-radius:5px; border:1px solid #ffcccc;">
    ...
    try {
        resultsArea.innerHTML = `<p style="color:#666">Analyzing...</p>`;
...
function displayResults(tasks) {
    ...
    tasks.forEach(task => {
        let color = '#00C851';
        if (task.score > 80) color = '#ff4444';
        else if (task.score > 40) color = '#ffbb33';

        const card = `
            <div style="border-left: 5px solid ${color}; background: #fff; padding: 15px;
                margin-bottom: 10px; border-radius: 5px; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
                
                <div style="float:right; background:#333; color:white; padding:2px 8px;
